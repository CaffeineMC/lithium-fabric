...
public class MeetVillagerTaskMixin {
    ...
    protected boolean shouldRun(ServerWorld world, LivingEntity entity) {
        ...
        if (world.getRandom().nextInt(100) == 0 && optional.isPresent() &&
                optional.get().getPos().isWithinDistance(entity.getPos(), 4.0D)) {

            if (world.getRegistryKey() == optional.get().getDimension()) {
    ...
    protected void run(ServerWorld world, LivingEntity entity, long time) {
        ...
        for (LivingEntity mob : visibleMobs) {
            if (EntityType.VILLAGER.equals(mob.getType()) && mob.squaredDistanceTo(entity) <= 32.0D) {
                // [VanillaCopy]
