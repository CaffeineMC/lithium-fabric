...
public abstract class MixinFindEntityTask<E extends LivingEntity, T extends LivingEntity> extends Task<E>  {
    @Shadow
    @Final
    ...
    private MixinFindEntityTask(Map<MemoryModuleType<?>, MemoryModuleState> requiredMemoryState) {
    ...
    private boolean method_24582(E livingEntity) {
        final List<LivingEntity> visibleMobs = livingEntity.getBrain().getOptionalMemory(MemoryModuleType.VISIBLE_MOBS)
    ...
    protected void run(ServerWorld world, E self, long time) {
        ...
        for (LivingEntity mob : visibleMobs) {
            if (this.entityType.equals(mob.getType())) {
                if (self.squaredDistanceTo(self) > (double) this.maxSquaredDistance) {
                    continue;
                }
